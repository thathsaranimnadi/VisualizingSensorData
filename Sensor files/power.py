...
def simulate_floor(floor, mean_R, mean_X, sampling_rate, duration):
    while True:
        ...
        print(f"Floor {floor} - Average Power: {avg_power:.2f} W")
        ...
        # Save data to files
        with open(f'floor_{floor}_voltage_data.json', 'w') as f:
            json.dump(voltage_data, f)
        
        with open(f'floor_{floor}_current_data.json', 'w') as f:
            json.dump(current_data, f)
        
        with open(f'floor_{floor}_power_meter_data.json', 'w') as f:
...
if __name__ == "__main__":
    ...
    threads = []
    for floor in floors:
        thread = threading.Thread(target=simulate_floor, args=(floor, mean_R, mean_X, sampling_rate, duration))
